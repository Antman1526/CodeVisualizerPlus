  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("codevisualizer.exportFlowchart", async () => {
      ...
      try {
        const pick = await vscode.window.showQuickPick(
          [
            { label: "$(file-media) PNG", description: "Export as a high-resolution PNG image", value: "png" as const },
            { label: "$(symbol-color) SVG", description: "Export as a scalable vector graphic", value: "svg" as const },
          ],
          { placeHolder: "Choose export format" }
        );
        if (!pick) return;

        // Try the panel first, then fall back to the sidebar
        const panelProvider = FlowchartPanelProvider.getInstance(context.extensionUri);
        if (panelProvider.isVisible()) {
          panelProvider.triggerExport(pick.value);
        } else {
          sidebarProvider.triggerExport(pick.value);
        }
      } catch (error) {
        console.error("Error in exportFlowchart:", error);
    ...
    vscode.commands.registerCommand("codevisualizer.visualizeCodebase", async (uri?: vscode.Uri) => {
      ...
      try {
        ...
        // Auto-open README.md preview if one exists in the selected folder
        const folderToCheck = uri
          ? uri.fsPath
          : vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
        if (folderToCheck) {
          const readmeCandidates = ["README.md", "readme.md", "Readme.md"];
          for (const name of readmeCandidates) {
            const readmePath = require("path").join(folderToCheck, name);
